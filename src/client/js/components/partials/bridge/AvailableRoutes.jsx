...
export default function AvailableRoutes(props) {
  ...
  const routes = _.map(props.routes, function (v, i) {
    ...
    if (bridgeType === 'connext' && !GENERIC_SUPPORTED_BRIDGE_TOKENS.includes(props.from.symbol.toUpperCase())) {
      route = route.concat([
        {
          type: 'swap',
          data: {
    ...
    route = route.concat([
      {
        type: 'token-network',
        amount: utils.formatUnits(v.estimate?.returnAmount ?? constants.Zero, props.to.decimals),
        token: props.to,
    ...
    >
      ...
      {routes.length > 0 &&
        _.map(routes, function (item, i) {
          return <RouteItemWrapper handleChange={props.handleChange} key={i} data={item} index={i}></RouteItemWrapper>;
        })}
    </div>
  );
}
